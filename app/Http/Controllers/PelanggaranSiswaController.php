        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = $btn . '<a href="javascript:void(0)" data-id="' . $row->id . '" class="btn btn-danger btn-sm delete"><i class="tf-icons bx bx-trash"></i></a>';
                    return '<center>' . $btn . '</center>';
                })
                ->rawColumns(['action', 'nama_pelanggaran', 'poin', 'status', 'tanggal'])
                ->make(true);
        }
        return view('pelanggaran-siswa.points', compact('menu', 'pelanggaran', 'id', 'siswa', 'rombel'));
    }

    public function show($id)
    {
        $data = PelanggaranSiswa::with(['siswa', 'rombel', 'pelanggaran']) // Eager load siswa and rombel
            ->where('id', $id)
            ->first();
