        ...
        <x-modal size="modal-lg">
            <div class="row g-3">
                <div class="col mb-0">
                    ...
                    <x-input type="text" name="tujuan" label="Tujuan Surat" value=""></x-input>
                </div>
                <div class="col mb-0">
    ...
    @section('script')
        <script text="javascript">
            $(function() {
                ...
                var myTable = DataTable("{{ route('surat-keluar.index') }}", [{
                        "data": null,
                        "orderable": false,
                        "searchable": false,
                ...
                // Edit
                var editUrl = "{{ route('surat-keluar.index') }}";
                var editHeading = "Edit {{ $menu }}";
                var field = ['klasifikasi_id', 'nomor', 'sifat', 'perihal', 'tujuan', 'tgl_surat'];
                editModel(editUrl, editHeading, field)

                // Save
                saveImage("{{ route('surat-keluar.store') }}", myTable);

                // Delete
                var fitur = "{{ $menu }}";
                var editUrl = "{{ route('surat-keluar.index') }}";
                var deleteUrl = "{{ route('surat-keluar.store') }}";
                Delete(fitur, editUrl, deleteUrl, myTable)

                var dokumenUrl = "{{ url('surat-keluar/review') }}";
                var dokumenPath = "{{ asset('storage/surat-keluar') }}";
