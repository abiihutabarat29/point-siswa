...
@section('content')
    <form action="{{ route('rombel.store') }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
        ...
        <x-card>
            <div class="row g-2">
                <div class="col mb-0">
                    <x-input type="text" name="name" label="Nama Kelas / Rombel" value=""></x-input>
                    <x-dropdown name="jurusan_id" label="Jurusan">
                        @foreach ($jurusan as $jur)
                            <option value="{{ $jur->id }}" @selected(old('jurusan_id') == $jur->id)>{{ $jur->name }}
                            </option>
                        @endforeach
                    </x-dropdown>
                </div>
                <div class="col mb-0">
                    <x-dropdown name="kelas_id" label="Kelas">
                        @foreach ($kelas as $kel)
                            <option value="{{ $kel->id }}" @selected(old('kelas_id') == $kel->id)>{{ $kel->name }}
                            </option>
                        @endforeach
                    </x-dropdown>
                </div>
...
@section('script')
    <script text="javascript">
        $(document).ready(function() {
            ...
            // Fungsi untuk mengupdate jumlah siswa dan opsi ketua kelas
            ...
            // Fungsi untuk menambahkan opsi ketua kelas
            function ketuaKelas() {
                $("#siswa_id").empty();
            ...
            // Panggil fungsi ketuaKelas saat halaman dimuat
            ketuaKelas();
